...
const examineText =
  'On the table by the door, where there was nothing last night: a speaker the size of a loaf, a motorcycle battery, and a grey box with two sockets in it, all three of them strapped together with duct tape into an object that is either very badly made or exactly as well made as it needs to be.\n\nThe strapping has been done in one continuous wind, corner to corner to corner, by somebody who did it once rather than four times. There is a loop of webbing on the top of it at exactly the height of a hand.\n\nIt has one socket the shape of the stick, and it is the only other thing in this county that has.';

const dropRefusedText = 'You are not putting this down in a road. You spend a second working out why not, and then stop working it out.';
