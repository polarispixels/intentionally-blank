...
const firstSightText =
  'The ladder takes you down out of the tunnel wall into a shaft that is not the same age as the tunnel is.\n\nThe top of it is the concrete of a building put up quickly by people who were being paid by the week: the grain of the shuttering still in it, board by board, with a tide line of whitewash about a third of the way down where somebody stopped and did not come back.\n\nBelow the tide line the wall is not poured and it is not block. It is smooth all the way round, and it is warm, and it goes down past the edge of your light without a joint in it anywhere. The ladder is bolted to it in the ordinary way, with ordinary bolts, through ordinary plates, and that is the strangest thing in the shaft.\n\nReturn B comes down the corner behind the ladder, turns once a little below the level of your boots, and goes into the wall, and does not come out.\n\nAcross from you at about that height there is a lift landing: two steel leaves meeting on a rubber seal, with the ordinary chamfer round them, and no call button on this side of them at all.\n\nThe ladder goes on down.';

const unconditionalText =
  'The shaft. Boarded concrete above the whitewash line and something with no joints in it below, and a ladder bolted to both as if there were no difference.\n\nUp is the tunnel. Down is where the ladder is going. There is a stencil on the smooth part of the wall, a lift landing with nothing to press, and a warm pipe going into a wall it does not come out of.';
...
const listenText =
  'Water, a long way off and directly below, going through something at a steady rate and not varying by the width of a hair, and behind that a sound like a room being large, which is already there before you start listening for it and stops the moment you notice you have.';

const smellText =
  'Warm dust off the concrete for the top third of it, and then nothing at all. Not cold, not clean, not damp. The smooth part of this shaft smells of nothing whatsoever, which is a thing surfaces do not generally manage.';

const yellText = 'It goes up and comes back off the boarding, and it goes down and does not come back off anything.';

const sleepWaitText = 'Halfway down a ladder in a hole under a county, with the light you brought and the batteries you have got. No.';
...
const ladderExamineText =
  'Steel, bolted through the wall in fours, with the rungs worn bright on top and untouched underneath.\n\nThe bolts through the boarded concrete are rusted at the heads and painted over. The bolts through the smooth part are not rusted, not painted, and not newer. They are the same bolts, and they have been in that wall for exactly as long, and one half of the wall has aged and the other has not.';

/** §5 — "CLIMB DOWN"/bare `DOWN`, this room's own `down` exit's `travelText`. */
export const LADDER_CLIMB_DOWN_TEXT =
  'The rungs go on being rungs for longer than you were expecting, and the light you have got throws your own shadow down the wall ahead of you the whole way, which is company of a sort.';
...
export const LADDER_CLIMB_UP_TEXT =
  'Up past the lift landing, up past the stencil, up past the tide line, and out through a hatch into a mile of poured tube with rails in the floor of it.';
...
const climbUpReconciliationText =
  'Up the ladder, past the lift landing with nothing to press, past the stencil, past the tide line, and out into a mile of poured tube with the air on your back.\n\nIt takes a while. Nothing at all happens while it is taking it.';
...
const stencilExamineText =
  'On the smooth part of the wall at head height, in white, sprayed through a stencil that somebody held a little crooked:\n\n    SYSTEM REVISION 2089.4\n\nThe overspray has feathered where the stencil lifted at one corner. The paint has not aged.\n\nNeither has the wall it is on, so there is nothing here to compare it to.';

const stencilTouchText = 'It is in the surface rather than on it. Your thumb comes away white and the letters do not change.';
...
const pipeExamineText =
  "Twelve inches, bare steel from a yard below the Sublevel 5 floor and bare all the way down here, coming past the ladder in the corner and turning once into the smooth wall a little below the level of your boots.\n\nIt arrives, and it goes in, and there is no flange, no seal, no collar and no made-good where it goes in. The wall is closed round it the way skin is closed round a splinter.";

const pipeTouchText =
  "Warm. Blood warm, or a shade under it, the same as it was five floors up in a plant room with a drawing on the wall that does not have this pipe on it.";
...
const liftExamineText =
  'Two steel leaves meeting on a rubber seal, in a frame with the same chamfer as every lift landing in the building above you.\n\nThere is no call button on this side, and no place a call button was taken out of. There is no floor number over the frame, and no place one was.\n\nThere is a slot the width of a screwdriver where the leaves meet, at the height a man\'s hands go, and the paint round it is unmarked.';

const liftOpenText =
  'The leaf you get the leg behind moves about the width of a card and then meets whatever holds a lift landing shut when the car is not there, which is designed to hold and does.\n\nThrough the gap: a shaft, and a rail, and a great deal of air, and no car in either direction as far as your light will go.\n\nYou take the leg out and the gap closes itself, unhurried, the way it was built to.';

const liftKnockText =
  'The knock goes into the lift shaft and comes back off four walls at four different times, which is more information about the shaft than you wanted and none at all about the floor.';
...
const stairDoorExamineOpenText =
  'A steel fire door standing open on a hook, at the bottom of a flight of poured steps that come down out of a light you can see from here and have walked in.\n\nIt has been open since a man in a suit went back up it and got into a lift with somebody holding his elbow.';

/** §9.2 — also this room's own `e` exit's `blockedText`, below. */
export const STAIR_DOOR_EXAMINE_SHUT_TEXT =
  'A steel fire door in the shaft wall, shut, with a closer on it and no handle on this side.\n\nSomewhere behind it there is a stair, and at the top of the stair a door that wants two things, and you have got neither of them and never had.';
