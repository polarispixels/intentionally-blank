...
const SHOP_OPEN_TEXT =
  'The shop door stands open on a hook, and the card on the sucker has been turned round to the side that says OPEN in the same hand.\n\nInside is one long room that smells of sacking and paraffin and coffee, which you have had through the gap under this door in the dark and which turns out to be exactly right. Shelves to the ceiling both sides. The ladder on its rail has been left where somebody last needed it and not put back. The bulb over the counter is on, because it is always on.\n\nThere is nobody in it. Not nobody behind the counter — nobody, in the way a room is empty when the person who runs it has gone to do something and has not thought about it since.\n\nOn the counter, where the till would be if there were a till: a cigar box with a slot cut in the lid.';
